## FaceOffChunk 1.0
## A node for InvokeAI, written by YMGenesis/Matthew Janik & JPPhoto/Jonathan
...
@invocation_output("face_off_chunk_output")
class FaceOffChunkOutput(BaseInvocationOutput):
...
@invocation("face_off_chunk", title="FaceOffChunk", tags=["image", "faceoff", "face", "mask"], category="image", version="1.0.0")
class FaceOffChunkInvocation(BaseInvocation):
    """bound, extract, and mask one face found from chunking an image using MediaPipe detection. Use when a face is too small for FaceOff."""
    ...
    def faceoff(self, context: InvocationContext) -> FaceOffChunkOutput:
    ...
    def invoke(self, context: InvocationContext) -> FaceOffChunkOutput:
        try:
