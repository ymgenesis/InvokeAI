...
class ChunkedSlicedAttnProcessor:
    ...
    def __call__(self, attn, hidden_states, encoder_hidden_states=None, attention_mask=None):
        ...
        for i in range(batch_size_attention // self.slice_size):
            ...
            #print("pre get_attention_scores_chunked")
            ...
            #attn_slice = torch.zeros((attn_slice_chunks[0].shape[0], attn_slice_chunks[0].shape[1], value.shape[2]))
            #results = []
            #print("pre while")
            #while len(attn_slice_chunks) > 0:
            #    results.append(torch.bmm(attn_slice_chunks.pop(), value[start_idx:end_idx]))
            #print("pre cat")
            #torch.cat(results, out=hidden_states[start_idx:end_idx])
            #print("post cat")
    ...
    def get_attention_scores_chunked(self, attn, query, key, attention_mask, hidden_states, value, chunk):
        ...
        #print(f"{chunks_count=} {out_size=}")
        # (b, n, p) -> (b, n1, p), (b, n2, p) where sum(n_i) = n
        chunk_step = max(1, int(query.shape[1] / chunks_count))
        #query_chunks = torch.split(query, math.ceil(query.shape[0] / chunks_count), dim=0)
        #if attention_mask is not None:
        #    input_chunks = torch.split(attention_mask, math.ceil(attention_mask.shape[0] / chunks_count), dim=0)

        key = key.transpose(-1, -2)

        #pos = 0
        #chunk = torch.zeros(query.shape[0], query.shape[1], key.shape[2], dtype=query.dtype, device=query.device)
        ...
        for chunk_pos in range(0, query.shape[1], chunk_step):
            if attention_mask is not None:
                torch.baddbmm(
                    #input_chunks[chunk_num],
                    #query_chunks[chunk_num],
                    _get_chunk_view(attention_mask, chunk_pos, chunk_step),# attention_mask[:,chunk_step*chunk_num:chunk_step*(chunk_num + 1)],
                    _get_chunk_view(query, chunk_pos, chunk_step), #query[:,chunk_step*chunk_num:chunk_step*(chunk_num + 1)],
                    key,
                    beta=1,
                    alpha=attn.scale,
                    out=chunk,
            ...
            #print(f"{hidden_states.shape=} {chunk.shape=}")
            # hidden_states.shape=torch.Size([1, 4096, 40]) chunk.shape=torch.Size([1, 128, 4096])
            torch.bmm(chunk, value, out=_get_chunk_view(hidden_states, chunk_pos, chunk_step)) #hidden_states[:,pos:pos+chunk.shape[1]])
            #pos += chunk.shape[1]
